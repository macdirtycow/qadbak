#!/usr/bin/env bash
# Remove Webmin / VirtualMin after Qadbak native mode is verified.
# Keeps nginx, Apache, Postfix, Dovecot, MariaDB, BIND, and customer data.
#
# Usage:
#   sudo bash scripts/uninstall-virtualmin.sh
#   sudo bash scripts/uninstall-virtualmin.sh --yes     # skip confirmation
...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "Run as root: sudo bash scripts/uninstall-virtualmin.sh" >&2
  exit 1
fi

echo "============================================"
echo " Uninstall Webmin / VirtualMin packages"
echo "============================================"
echo " Qadbak panel and customer sites (nginx, mail, DB) are NOT removed."
...
if [[ -f "$QADBAK_DIR/scripts/preflight-phase8-independent.sh" ]]; then
  if ! bash "$QADBAK_DIR/scripts/preflight-phase8-independent.sh"; then
    echo "Preflight failed — fix the panel before removing Webmin." >&2
...
if command -v apt-get >/dev/null 2>&1; then
  ...
  echo "Unknown package manager — remove webmin/virtualmin packages manually." >&2
  exit 1
fi
...
if [[ -f "$ENV_FILE" && "$DRY" -ne 1 ]]; then
  ...
  echo "[dry-run] strip VIRTUALMIN_* / WEBMIN_* from .env.local; set native provisioner"
fi

echo "==> Panel nginx (no /embed/webmin/)"
...
echo "  Customer sites: unchanged (native nginx/Apache/mail)."
echo "  Panel health: curl -s http://127.0.0.1:3000/api/health"
echo "  Optional cleanup: rm -rf /etc/webmin /usr/share/webmin /var/webmin"
