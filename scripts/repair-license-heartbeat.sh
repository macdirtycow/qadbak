...
PUBLIC_URL="${QADBAK_LICENSE_SERVER:-https://license.inveil.dev}"
...
if ! grep -q '^QADBAK_LICENSE_SERVER=' "$ENV_FILE" 2>/dev/null; then
  echo "==> Set QADBAK_LICENSE_SERVER=$PUBLIC_URL"
  upsert_env QADBAK_LICENSE_SERVER "$PUBLIC_URL"
fi
...
if curl -sf --max-time 10 "$PUBLIC_URL/health" | tee /tmp/qadbak-license-health.json | grep -q '"ok"'; then
  echo "  OK"
else
...
if ! grep -q '^QADBAK_LICENSE_JWT_SECRET=' "$ENV_FILE" 2>/dev/null; then
  echo ""
  echo "  FAIL — QADBAK_LICENSE_JWT_SECRET missing in .env.local" >&2
  echo "  Must match LICENSE_JWT_SECRET on the license server (/etc/qadbak/license-server.env)." >&2
  exit 1
fi
...
if [[ "$HB_RC" -ne 0 ]]; then
  ...
  if grep -q 'Not found' <<<"$HB_OUT"; then
    ...
    echo "  Ensure QADBAK_LICENSE_SERVER=$PUBLIC_URL (no /buy suffix)." >&2
