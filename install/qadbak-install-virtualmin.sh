...
echo "  LEGACY installer — installs VirtualMin and Webmin on this server."
read -rp "Continue with VirtualMin install? [y/N]: " CONFIRM_VM
if [[ ! "$CONFIRM_VM" =~ ^[Yy]$ ]]; then
  echo "Aborted. For native install run: sudo bash install/qadbak-install.sh"
  exit 0
fi
